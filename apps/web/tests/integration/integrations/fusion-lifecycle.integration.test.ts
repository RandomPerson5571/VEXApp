...
import { DELETE, GET, PATCH } from "@/app/api/team/fusion/route";

const describeIntegration = hasTestDatabase() ? describe : describe.skip;

describeIntegration("Fusion integration lifecycle", () => {
  let teamId = "";
  let userId = "";
  ...
  it("GET returns the seeded integration", async () => {
    await createTestFusionIntegration(teamId, {
      projectUrn: "urn:adsk.wipprod:fs.folder:co.lifecycle",
      isActive: true,
    });
    ...
    expect(body.integration.projectUrn).toBe(
      "urn:adsk.wipprod:fs.folder:co.lifecycle",
    );
  });

  it("PATCH toggles isActive", async () => {
    await createTestFusionIntegration(teamId, { isActive: true });

    const response = await PATCH(
      new Request("https://example.test/api/team/fusion", {
        method: "PATCH",
