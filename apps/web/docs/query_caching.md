...
| Documentation tree | `queryKeys.docs.tree(teamId)` | team |
| Document detail | `queryKeys.docs.detail(docId)` | document |
...
|---|---|---|
| `/dashboard` | `prefetchDashboard` | events, dashboard tasks, inventory, summary |
| `/task-list` | `prefetchTeamTasks` + `prefetchTeamMembers` | full task list, assignees |
| `/documents` | `prefetchTeamDocumentation` | folder tree |
...
Helpers in `lib/queries/cache-updates/` keep mutation logic out of components. Pure merge functions (`mergeTaskInList`, `mergeDocInTree`, etc.) are unit-tested in `tests/unit/queries/cache-updates.test.ts`.
...
### Documentation — patch then invalidate

`lib/hooks/use-team-documentation.ts` + `cache-updates/documentation.ts` + `cache-updates/folders.ts`

| Mutation | Immediate cache write | `onSettled` |
|---|---|---|
| Update document | `applyDocumentationDetailPatch` + `applyDocumentationTreeDocPatch` | `invalidateDocsTree` (+ detail key when `docId` known) |
| Update folder | `applyFolderTreePatch` | `invalidateDocsTree` |
| Create / delete folder or document | — | `invalidateDocsTree` only |

Structural changes (create/delete) are invalidate-only — too error-prone to patch by hand. Title/type edits get an instant UI update, then a background refetch reconciles.
...
| `invalidateDocsTree(teamId, docId?)` | `docs.tree`; optionally `docs.detail(docId)` |
...
| Documentation mutation hooks | `lib/hooks/use-team-documentation.ts` |
