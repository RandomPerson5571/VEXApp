...
function InventoryRow({ item, index }: { item: TeamInventoryItem; index: number }) {
  ...
  return (
    <div
      className="group relative overflow-hidden rounded-3xl border border-white/10 bg-slate-950/60 p-3 transition-[transform,opacity,border-color] duration-300 motion-safe:hover:-translate-y-0.5 motion-safe:hover:border-white/20 motion-reduce:transition-none"
      ...
      <div className="relative flex items-start justify-between gap-3">
        ...
        <div className="min-w-0 flex-1 space-y-2">
          ...
          <div className="space-y-1">
            ...
            <div className="h-1.5 w-full overflow-hidden rounded-full border border-white/10 bg-slate-900/80">
              <div
...
export function InventoryTrackerWidget() {
  ...
  return (
    <div className="lg:col-span-5 relative flex flex-col overflow-hidden rounded-[32px] border border-white/10 bg-[#091126]/80 p-6 shadow-[0_24px_80px_rgba(0,0,0,0.35)]">
      <div
        aria-hidden
      ...
      <div className="relative mb-4 flex items-start justify-between gap-3 border-b border-white/10 pb-3">
        <div>
      ...
      <div className="relative mb-3 flex flex-col gap-2 sm:flex-row sm:items-center">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full rounded-3xl border border-white/10 bg-slate-950/80 py-2.5 pl-10 pr-3 text-[11px] font-semibold text-slate-100 outline-none transition focus:border-blue-400/40 focus:shadow-[0_0_0_3px_rgba(59,130,246,0.16)] placeholder:text-slate-500"
          />
        </div>
      </div>
      ...
      <div className="relative flex-1 space-y-2.5 overflow-y-auto pr-0.5 dashboard-scroll max-h-[340px]">
        {isLoading ? (
          Array.from({ length: 4 }).map((_, index) => (
            <div
              key={index}
              className="h-20 animate-pulse rounded-3xl border border-white/10 bg-slate-950/60"
            />
          ))
        ) : filteredItems.length === 0 ? (
          <div className="flex flex-col items-center justify-center rounded-3xl border border-white/10 bg-slate-950/60 py-10 text-center">
            <Package className="mb-2 h-8 w-8 text-slate-400" />
            <p className="text-xs font-bold text-white">No matching parts</p>
...
function SummaryPill({
  ...
  return (
    ...
    >
      ...
      <span className="text-lg font-black font-mono leading-none text-white">{value}</span>
    </div>
  );
}
