...
function InventoryRow({ item, index }: { item: TeamInventoryItem; index: number }) {
  ...
  return (
    <div
      className="group relative overflow-hidden rounded-xl border border-slate-900 bg-slate-950/50 p-3 transition-[transform,opacity,border-color] duration-300 motion-safe:hover:-translate-y-0.5 motion-safe:hover:border-slate-800 motion-reduce:transition-none"
      ...
      <div className="relative flex items-start justify-between gap-3">
        ...
        <div className="min-w-0 flex-1 space-y-2">
          <div className="flex flex-wrap items-center gap-2">
            <p className="truncate text-xs font-extrabold text-slate-100">{item.name}</p>
            {status !== "nominal" ? (
              <span
          ...
          <div className="space-y-1">
            ...
            <div className="h-1.5 w-full overflow-hidden rounded-full border border-slate-900/80 bg-slate-950">
              <div
        ...
        <ChevronRight className="mt-1 h-4 w-4 shrink-0 text-slate-700 transition-colors duration-200 group-hover:text-slate-500" />
      </div>
    </div>
  );
}
...
export function InventoryTrackerWidget() {
  ...
  return (
    <div className="lg:col-span-5 relative flex flex-col overflow-hidden rounded-2xl border border-slate-900/80 bg-[#090e18]/80 p-6 shadow-md backdrop-blur-sm">
      <div
        aria-hidden
      ...
      <div className="relative mb-4 flex items-start justify-between gap-3 border-b border-slate-900 pb-3">
        <div>
        ...
        <Link
          href="/inventory"
          className="text-[10px] font-bold text-blue-500 transition-colors hover:text-blue-400 hover:underline"
        >
          Manage Inventory →
        </Link>
      </div>
      ...
      <div className="relative mb-3 flex flex-col gap-2 sm:flex-row sm:items-center">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full rounded-lg border border-slate-900 bg-slate-950/80 py-2 pl-8 pr-3 text-[11px] font-semibold text-slate-200 outline-none transition-[border-color,box-shadow] placeholder:text-slate-600 focus:border-blue-500/40 focus:shadow-[0_0_0_3px_rgba(59,130,246,0.12)]"
          />
        </div>
      </div>
      ...
      <div className="relative flex-1 space-y-2.5 overflow-y-auto pr-0.5 dashboard-scroll max-h-[340px]">
        {isLoading ? (
          Array.from({ length: 4 }).map((_, index) => (
            <div
              key={index}
              className="h-20 animate-pulse rounded-xl border border-slate-900 bg-slate-950/50"
            />
          ))
        ) : filteredItems.length === 0 ? (
          <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-slate-900 py-10 text-center">
            <Package className="mb-2 h-8 w-8 text-slate-700" />
            <p className="text-xs font-bold text-slate-400">No matching parts</p>
...
function SummaryPill({
  ...
  return (
    ...
    >
      ...
      <span className="text-lg font-black font-mono leading-none text-slate-100">{value}</span>
    </div>
  );
}
